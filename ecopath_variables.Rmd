...
mapa.locs <- ggplot(data = world) +
    geom_sf() +
    geom_sf(data = shape.list, fill="yellow") + 
    coord_sf(xlim = c(-119, -70), ylim = c(-10, 35), expand = FALSE) +
...
model.plot <- model.plot.data %>% 
      ggplot(aes(x=Variable, y=value_res, group=model)) + 
...
#Ascendency, Overhead and the Overhead/ Capacity ratio 


summary.ind <- read_xlsx("Summary models PiO.xlsx", sheet = 1) %>% 
   pivot_longer(cols = -Variable, names_to = "model",values_to = "value") %>% 
  dplyr::filter(Variable %in% c("Ascendency","Overhead", "O/C", "Connectance Index")) %>% 
  mutate(value = if_else(is.na(value),0,value)) %>% 
  group_by(Variable) %>% 
 # resca(value, new_min = 0, new_max = 1) %>% 
 # dplyr::select(-value) %>% 
...
pca.axes <- res.pca$x %>% 
  ...
  dplyr::rename(OC=`O/C`, ci_index=`Connectance Index`)
...
pc.gam.tp.as <- gam(Ascendency ~ 
                   # s(rec_index, bs = 're') ,
                    s(PC1, PC2, PC3, bs = base, k=10), #PC axis are on the same scale
                   # s(PC2, bs=base, k=8),
            #        s(PC3, bs=base, k=8),
                  family=gaussian(link="identity"), method="REML", gamma=1.4, select=TRUE, data=pca.axes)

saveRDS(pc.gam.tp.as,"pc_gam_tp_ascendency.rds")

gam.check(pc.gam.tp.as)

try(concurvity(pc.gam.tp.as, full = TRUE))

summary(pc.gam.tp.as)


base="ts"

pc.gam.ts.as <- gam(Ascendency ~ 
                   # s(rec_index, bs = 're') ,
                    s(PC1, PC2, PC3, bs = base, k=10), #PC axis are on the same scale
                   # s(PC2, bs=base, k=8),
            #        s(PC3, bs=base, k=8),
...
saveRDS(pc.gam.ts.as,"pc_gam_tp_ascendency.rds")

gam.check(pc.gam.ts.as)

try(concurvity(pc.gam.ts.as, full = TRUE))

summary(pc.gam.ts.as)


anova.gam(pc.gam.tp.as, pc.gam.ts.as, test="F")

aic.model.values <- AIC(pc.gam.tp.as, pc.gam.ts.as) %>% 
  arrange(desc(AIC))
...
saveRDS(pc.gam.tp.ov,"pc_gam_tp_ascendency.rds")

gam.check(pc.gam.tp.ov)

try(concurvity(pc.gam.tp.ov, full = TRUE))

summary(pc.gam.tp.ov)


base="ts"

pc.gam.ts.ov <- gam(Overhead ~ 
                   # s(rec_index, bs = 're') ,
                    s(PC1, PC2, PC3, bs = base, k=10), #PC axis are on the same scale
                   # s(PC2, bs=base, k=8),
            #        s(PC3, bs=base, k=8),
...
saveRDS(pc.gam.ts.ov,"pc_gam_tp_ascendency.rds")

gam.check(pc.gam.ts.ov)

try(concurvity(pc.gam.ts.ov, full = TRUE))

summary(pc.gam.ts.ov)
...
saveRDS(pc.gam.tp.oc,"pc_gam_tp_ascendency.rds")

gam.check(pc.gam.tp.oc)

try(concurvity(pc.gam.tp.oc, full = TRUE))

summary(pc.gam.tp.oc)
...
saveRDS(pc.gam.ts.oc,"pc_gam_tp_ascendency.rds")

gam.check(pc.gam.ts.oc)

try(concurvity(pc.gam.ts.oc, full = TRUE))

summary(pc.gam.ts.oc)

anova.gam(pc.gam.tp.oc, pc.gam.ts.oc, test="F")

aic.model.values <- AIC(pc.gam.tp.oc, pc.gam.ts.oc) %>% 
  arrange(desc(AIC))
...
pc.gam.tp.ci <- gam(ci_index ~ 
                   # s(rec_index, bs = 're') ,
                    s(PC1, PC2, PC3, bs = base, k=10), 
                  family=gaussian(link="identity"), method="REML", gamma=1.4, select=TRUE, data=pca.axes)


#glm

pc.glm.tp.ci <- glm(ci_index ~ PC1 + PC2 + PC3, family= gaussian, data=pca.axes)

saveRDS(pc.gam.tp.ci,"pc_gam_tp_connindx.rds")
...
herring.viz.term.5 <- plot(sm(herring.viz, 5)) +
  ...
  l_fitLine(colour = "black") + 
    #l_rug(mapping = aes(x=x, y=y), alpha = 0.8) +
   # l_ciLine(mul = 5, colour = "gray85", linetype = 2) + 
  #  l_points(shape = 19, size = 1, alpha = 0.1) + 
  theme_classic() +
  labs(tag = "E") +
