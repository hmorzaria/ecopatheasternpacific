...
mapa.lme <- ggplot(data = countries) +
  ...
  geom_sf(data = lme.pol, aes(fill=LME_NAME)) +
   geom_text(aes(x = -103, y = 25, label = "Mexico"), stat = "unique", size = 5) +  
...
mapa.secc1 <- ggplot(data = countries) +
  ...
  geom_text_repel(data = eco.north, aes(x = long, y = lat, label = model), 
        fontface = "plain", nudge_x = c(-1.5, -1.5, 1, 1, 1,-2,-2,1,2,-2,-1), nudge_y = c(-0.75, -0.25, -0.5,0,0,0.5,0,0,0.5,-0.5,0), size = 3)
...
mapa.secc2 <- ggplot(data = countries) +
  geom_sf() +  
  geom_sf(data = lme.pol, aes(fill=LME_NAME)) +
  geom_text(aes(x = -78, y = -1, label = "Ecuador"), stat = "unique", size = 5) +  
  scale_fill_manual(values=col.pal, name="LME") +
  ...
  annotation_scale(location = "bl", width_hint = 0.4) +
  theme(legend.position="none") +
  geom_text_repel(data = eco.south, aes(x = long, y = lat, label = model), 
        fontface = "plain", nudge_x = c(1, -1.5), nudge_y = c(1, 0.25), size = 3)
...
lme.points <- read_csv(here("lme_model_points.csv")) %>% 
  ...
  mutate(LME_NAME = if_else(model == "Isla del Coco" | model == "Darwin and Wolf", "None", LME_NAME))
